    ...
    with tabs_budget[0]:
        ...
        if bs and bs.get('year') == year:
            ...
            with st.container(border=True):
                ...
                for col in editable_cols:
                    if col not in df_formation.columns:
                        df_formation[col] = 0 if col != 'Dénomination' else ''

                df_formation_edit = df_formation[editable_cols].copy()
                df_formation_edit['Effectif (pers.)'] = pd.to_numeric(df_formation_edit['Effectif (pers.)'], errors='coerce').fillna(0).astype(int).clip(lower=0)
                df_formation_edit['Heures'] = df_formation_edit['Heures'].apply(lambda x: float(str(x).replace(',', '.')) if pd.notna(x) else 0.0).clip(lower=0)
                df_formation_edit['Nbre de shifts'] = pd.to_numeric(df_formation_edit['Nbre de shifts'], errors='coerce').fillna(0).astype(int).clip(lower=0)

                column_config_formation = {
                    'Dénomination': st.column_config.TextColumn('Dénomination', required=True, help='Description du shift de formation'),
                    'Effectif (pers.)': st.column_config.NumberColumn('Effectif (pers.)', min_value=0, step=1, format='%d', required=True),
                    'Heures': st.column_config.NumberColumn('Heures', min_value=0.0, step=0.5, format='%.1f', required=True, help='Durée en heures (décimales acceptées)'),
                    'Nbre de shifts': st.column_config.NumberColumn('Nbre de shifts', min_value=0, step=1, format='%d', required=True),
                }
                ...
                edited_formation['Heures'] = edited_formation['Heures'].apply(lambda x: float(str(x).replace(',', '.')) if pd.notna(x) else 0.0).clip(lower=0)
                ...
                with col_btn1:
                    if st.button("➕ Ajouter une ligne", key="btn_add_formation"):
                        ...
                        st.session_state.budget_formation_at = pd.concat([st.session_state.budget_formation_at, new_row], ignore_index=True)
                        st.rerun()
                with col_btn2:
                ...
                with col_btn3:
                    if st.button("🗑️ Vider la table", key="btn_clear_formation"):
                        st.session_state.budget_formation_at = pd.DataFrame(columns=['Dénomination', 'Effectif (pers.)', 'Heures', 'Nbre de shifts'])
                        st.rerun()
            ...
            with st.container(border=True):
                ...
                for col in editable_cols:
                    if col not in df_formateurs.columns:
                        df_formateurs[col] = 0 if col != 'Dénomination' else ''

                df_formateurs_edit = df_formateurs[editable_cols].copy()
                df_formateurs_edit['Effectif (pers.)'] = pd.to_numeric(df_formateurs_edit['Effectif (pers.)'], errors='coerce').fillna(0).astype(int).clip(lower=0)
                df_formateurs_edit['Heures'] = df_formateurs_edit['Heures'].apply(lambda x: float(str(x).replace(',', '.')) if pd.notna(x) else 0.0).clip(lower=0)
                df_formateurs_edit['Nbre de shifts'] = pd.to_numeric(df_formateurs_edit['Nbre de shifts'], errors='coerce').fillna(0).astype(int).clip(lower=0)

                column_config_formateurs = {
                    'Dénomination': st.column_config.TextColumn('Dénomination', required=True, help='Description du shift formateur'),
                    'Effectif (pers.)': st.column_config.NumberColumn('Effectif (pers.)', min_value=0, step=1, format='%d', required=True),
                    'Heures': st.column_config.NumberColumn('Heures', min_value=0.0, step=0.5, format='%.1f', required=True, help='Durée en heures (décimales acceptées)'),
                    'Nbre de shifts': st.column_config.NumberColumn('Nbre de shifts', min_value=0, step=1, format='%d', required=True),
                }
                ...
                edited_formateurs['Heures'] = edited_formateurs['Heures'].apply(lambda x: float(str(x).replace(',', '.')) if pd.notna(x) else 0.0).clip(lower=0)
                ...
                with col_btn1:
                    if st.button("➕ Ajouter une ligne", key="btn_add_formateurs"):
                        ...
                        st.session_state.budget_formateurs_at = pd.concat([st.session_state.budget_formateurs_at, new_row], ignore_index=True)
                        st.rerun()
                with col_btn2:
                ...
                with col_btn3:
                    if st.button("🗑️ Vider la table", key="btn_clear_formateurs"):
                        st.session_state.budget_formateurs_at = pd.DataFrame(columns=['Dénomination', 'Effectif (pers.)', 'Heures', 'Nbre de shifts'])
                        st.rerun()
            ...
            with st.container(border=True):
                ...
                # KPI Totaux (avec sous-lignes "dont ...")
                if total_heures_formation > 0 or total_heures_formateurs > 0:
                    ...
                    breakdown_heures = (
                        "<div class='sub'>"
                        "dont<br>"
                        f"{total_heures_formation:,.1f} heure{' ' if abs(total_heures_formation-1)<1e-9 else 's '}Formation/Doublure AT<br>"
                        f"{total_heures_formateurs:,.1f} heure{' ' if abs(total_heures_formateurs-1)<1e-9 else 's '}de shifts ATF"
                        "</div>"
                    )
                    breakdown_cout = (
                        "<div class='sub'>"
                        ...
                        f"{total_cout_formateurs:,.0f} CHF de shifts ATF"
                        "</div>"
                    )
                ...
                with tab_cost:
                    ...
                    # Ajout Formation/Doublure AT & ATF
                    ...
                    if not summary.empty:
                        ...
                        with col2:
                            sel = alt.selection_point(fields=['Catégorie'], on='click', toggle=True, empty='none')

                            # ---- Bars with % du total in tooltip
                            bars = (
                                alt.Chart(summary_m)
                                ...
                                .encode(
                                    ...
                                    tooltip=[
                                        ...
                                        alt.Tooltip('pct:Q',    title='% du total',  format='.1%'),
                                    ],
                            ...
                            st.markdown("<span style='font-size:0.85em; font-style:italic; color:#555;'>💡 Astuce : Shift+Click pour sélectionner plusieurs barres ou en retirer.</span>", unsafe_allow_html=True)
                ...
                with tab_hours:
                    ...
                    if not calendar_df.empty:
                        ...
                        if 'total_heures_formation' in locals():
                            heures_df = pd.concat([heures_df, pd.DataFrame([{'Catégorie': 'Formation/Doublure AT', 'Heures': float(total_heures_formation)}])], ignore_index=True)
                        if 'total_heures_formateurs' in locals():
                            heures_df = pd.concat([heures_df, pd.DataFrame([{'Catégorie': 'ATF', 'Heures': float(total_heures_formateurs)}])], ignore_index=True)

                        heures_df = heures_df.groupby('Catégorie', as_index=False)['Heures'].sum()

                        col1h, col2h = st.columns([0.44, 0.56])
                        with col1h:
                            st.dataframe(heures_df.set_index('Catégorie').style.format({'Heures': '{:,.1f} h'}), use_container_width=True)

                        with col2h:
                            sel_h = alt.selection_point(fields=['Catégorie'], on='click', toggle=True, empty='none')
                ...
                # ---------------- DA OPT AT & Comptes internes
                st.markdown("---")
                calendar_df_all = bs.get('calendar_df', pd.DataFrame())

                # DA OPT AT heures (AT + CSC + EES + Formation)
                ...
                # DA OPT AT coût (Coût_AT + Coût_CSC + Coût_EES + coût formation)
                da_opt_at_cout = 0.0
                if not calendar_df_all.empty:
                ...
                sous_traitance_assistance = _sum_costs(calendar_df_all, ["AT"]) + _sum_costs(calendar_df_all, ["EES"] +  _sum_costs(calendar_df_all, ["ATR"] +  _sum_costs(calendar_df_all, ["ATF"] +  _sum_costs(calendar_df_all, ["Coordinateur"])
                    ...
                    try:
                        ...
                        with tab_monthly_cost:
                            ...
                            if category_cost_cols:
                                for col in category_cost_cols:
                                    df_costs[col] = pd.to_numeric(df_costs[col], errors='coerce').fillna(0.0).astype(float)
                            ...
                            for cat in category_cost_cols:
                                col_config_costs[cat] = st.column_config.NumberColumn(f"{cat} (CHF)", format="%.0f")
                            col_config_costs["Total"] = st.column_config.NumberColumn("Total (CHF)", format="%.0f")

                            st.dataframe(df_costs, column_config=col_config_costs, hide_index=True, use_container_width=True)
                        ...
                        with tab_monthly_hour:
                            ...
                            if category_hour_cols:
                                for col in category_hour_cols:
                                    df_hours[col] = pd.to_numeric(df_hours[col], errors='coerce').fillna(0.0).astype(float)
                            ...
                            for cat in category_hour_cols:
                                col_config_hours[cat] = st.column_config.NumberColumn(f"{cat} (h)", format="%.1f")
                            col_config_hours["Total"] = st.column_config.NumberColumn("Total (h)", format="%.1f")

                            st.dataframe(df_hours, column_config=col_config_hours, hide_index=True, use_container_width=True)
                        ...
                        if selected_month:
                            ...
                            tab_daily_cost, tab_daily_hour = st.tabs(
                                [f"Détail Jours {selected_month} (CHF)", f"Détail Jours {selected_month} (Heures)"]
                            )
                            ...
                            with tab_daily_cost:
                                ...
                                st.dataframe(df_daily_costs_view, column_config=daily_cost_config, hide_index=True, use_container_width=True)
                            ...
                            with tab_daily_hour:
                                ...
                                st.dataframe(df_daily_hours_view, column_config=daily_hour_config, hide_index=True, use_container_width=True)
                ...
                for i, key in enumerate(sorted_categories):
                    with cols[i % 3]:
                        ...
                        try:
                            ...
                            default_idx = 0
                        new_personnel = st.selectbox(f"**{key}** :", personnel_list, index=default_idx, key=f"map_{key}")
