...
from core.budget import (
    generate_budget_state,
    _ensure_adjusted_saisons_for_year,
    _season_timeline_df,
)
    ...
    with tabs_budget[0]:
        ...
        if "personnel" in st.session_state and not st.session_state.personnel.empty:
            ...
            with cols_act[0]:
                if st.button(
                    ...
                    key="generate_budget_button",
                ):
                    with st.spinner("Consolidation en cours…"):
                        generate_budget_state(year)
                    if (
                        "budget_state" in st.session_state
                        and st.session_state.budget_state.get("year") == year
                    ):
        ...
        if bs and bs.get("year") == year:
            ...
            with st.container(border=True):
                ...
                st.markdown(
                    "Gérez les shifts de formation et doublure AT indépendamment de la planification standard."
                )

                # Coût horaire AT
                cout_horaire_at = 45.50
                if "personnel" in st.session_state and not st.session_state.personnel.empty:
                    at_row = st.session_state.personnel[
                        st.session_state.personnel["Type"] == "AT"
                    ]
                    if not at_row.empty:
                        try:
                ...
                if "budget_formation_at" not in st.session_state:
                    st.session_state.budget_formation_at = pd.DataFrame(
                        [
                            {
                                "Dénomination": "Formation (théorique) AT S26",
                                "Effectif (pers.)": 20,
                                "Heures": 17.0,
                                "Nbre de shifts": 1,
                            },
                            {
                                "Dénomination": "Formation (pratique) AT S26",
                                "Effectif (pers.)": 20,
                                "Heures": 6.5,
                                "Nbre de shifts": 3,
                            },
                            {
                                "Dénomination": "Formation (théorique) AT CHT W26",
                                "Effectif (pers.)": 20,
                                "Heures": 17.0,
                                "Nbre de shifts": 1,
                            },
                            {
                                "Dénomination": "Formation (pratique) AT CHT W26",
                                "Effectif (pers.)": 20,
                                "Heures": 6.5,
                                "Nbre de shifts": 3,
                            },
                            {
                                "Dénomination": "Formation Visitor's Center",
                                "Effectif (pers.)": 20,
                                "Heures": 4.0,
                                "Nbre de shifts": 1,
                            },
                            {
                                "Dénomination": "Refresher AT CDI",
                                "Effectif (pers.)": 0,
                                "Heures": 0.0,
                                "Nbre de shifts": 0,
                            },
                            {
                                "Dénomination": "Cours FEU",
                                "Effectif (pers.)": 40,
                                "Heures": 3.0,
                                "Nbre de shifts": 1,
                            },
                            {
                                "Dénomination": "Cours DEFI",
                                "Effectif (pers.)": 40,
                                "Heures": 2.0,
                                "Nbre de shifts": 1,
                            },
                ...
                df_formation_edit = (
                    st.session_state.budget_formation_at[editable_cols].copy()
                    if set(editable_cols).issubset(st.session_state.budget_formation_at.columns)
                    else pd.DataFrame(columns=editable_cols)
                )

                # Normalisation
                df_formation_edit["Effectif (pers.)"] = (
                    pd.to_numeric(df_formation_edit.get("Effectif (pers.)", 0), errors="coerce")
                    .fillna(0)
                    .astype(int)
                    .clip(lower=0)
                )
                df_formation_edit["Heures"] = (
                    df_formation_edit.get("Heures", 0)
                    .apply(lambda x: float(str(x).replace(",", ".")) if pd.notna(x) else 0.0)
                    .clip(lower=0)
                )
                df_formation_edit["Nbre de shifts"] = (
                    pd.to_numeric(df_formation_edit.get("Nbre de shifts", 0), errors="coerce")
                    .fillna(0)
                    .astype(int)
                    .clip(lower=0)
                )
                ...
                column_config_formation = {
                    "Dénomination": st.column_config.TextColumn(
                        "Dénomination", required=True, help="Description du shift de formation"
                    ),
                    "Effectif (pers.)": st.column_config.NumberColumn(
                        "Effectif (pers.)", min_value=0, step=1, format="%d", required=True
                    ),
                    ...
                    "Nbre de shifts": st.column_config.NumberColumn(
                        "Nbre de shifts", min_value=0, step=1, format="%d", required=True
                    ),
                }
                ...
                edited_formation["Effectif (pers.)"] = (
                    pd.to_numeric(edited_formation.get("Effectif (pers.)", 0), errors="coerce")
                    .fillna(0)
                    .astype(int)
                    .clip(lower=0)
                )
                edited_formation["Heures"] = (
                    edited_formation.get("Heures", 0)
                    .apply(lambda x: float(str(x).replace(",", ".")) if pd.notna(x) else 0.0)
                    .clip(lower=0)
                )
                edited_formation["Nbre de shifts"] = (
                    pd.to_numeric(edited_formation.get("Nbre de shifts", 0), errors="coerce")
                    .fillna(0)
                    .astype(int)
                    .clip(lower=0)
                )
                ...
                col_btn1, col_btn2, col_btn3, _ = st.columns(4)
                with col_btn1:
                ...
                with col_btn3:
                    if st.button("🗑️ Vider la table", key="btn_clear_formation"):
                        st.session_state.budget_formation_at = pd.DataFrame(
                            columns=["Dénomination", "Effectif (pers.)", "Heures", "Nbre de shifts"]
                        )
                        st.rerun()
            ...
            # -------- Shift AT Formateurs planifiés (ATF)
            with st.container(border=True):
                ...
                if "personnel" in st.session_state and not st.session_state.personnel.empty:
                    atf_row = st.session_state.personnel[
                        st.session_state.personnel["Type"] == "ATF"
                    ]
                ...
                if "budget_formateurs_at" not in st.session_state:
                    st.session_state.budget_formateurs_at = pd.DataFrame(
                        [
                            ...
                            {
                                "Dénomination": "Doublure formation (pratique) AT S26",
                                "Effectif (pers.)": 20,
                                "Heures": 6.5,
                                "Nbre de shifts": 4,
                            },
                            ...
                            {
                                "Dénomination": "Doublure formation (pratique) AT CHT W26",
                                "Effectif (pers.)": 20,
                                "Heures": 6.5,
                                "Nbre de shifts": 4,
                            },
                            {
                                "Dénomination": "Doublure Visitor's Center",
                                "Effectif (pers.)": 20,
                                "Heures": 4.0,
                                "Nbre de shifts": 1,
                            },
                ...
                editable_cols = ["Dénomination", "Effectif (pers.)", "Heures", "Nbre de shifts"]
                df_formateurs_edit = (
                    st.session_state.budget_formateurs_at[editable_cols].copy()
                    if set(editable_cols).issubset(st.session_state.budget_formateurs_at.columns)
                    else pd.DataFrame(columns=editable_cols)
                )

                # Normalisation
                df_formateurs_edit["Effectif (pers.)"] = (
                    pd.to_numeric(df_formateurs_edit.get("Effectif (pers.)", 0), errors="coerce")
                    .fillna(0)
                    .astype(int)
                    .clip(lower=0)
                )
                df_formateurs_edit["Heures"] = (
                    df_formateurs_edit.get("Heures", 0)
                    .apply(lambda x: float(str(x).replace(",", ".")) if pd.notna(x) else 0.0)
                    .clip(lower=0)
                )
                df_formateurs_edit["Nbre de shifts"] = (
                    pd.to_numeric(df_formateurs_edit.get("Nbre de shifts", 0), errors="coerce")
                    .fillna(0)
                    .astype(int)
                    .clip(lower=0)
                )
                ...
                column_config_formateurs = {
                    "Dénomination": st.column_config.TextColumn(
                        "Dénomination", required=True, help="Description du shift formateur"
                    ),
                    "Effectif (pers.)": st.column_config.NumberColumn(
                        "Effectif (pers.)", min_value=0, step=1, format="%d", required=True
                    ),
                ...
                edited_formateurs["Effectif (pers.)"] = (
                    pd.to_numeric(edited_formateurs.get("Effectif (pers.)", 0), errors="coerce")
                    .fillna(0)
                    .astype(int)
                    .clip(lower=0)
                )
                edited_formateurs["Heures"] = (
                    edited_formateurs.get("Heures", 0)
                    .apply(lambda x: float(str(x).replace(",", ".")) if pd.notna(x) else 0.0)
                    .clip(lower=0)
                )
                edited_formateurs["Nbre de shifts"] = (
                    pd.to_numeric(edited_formateurs.get("Nbre de shifts", 0), errors="coerce")
                    .fillna(0)
                    .astype(int)
                    .clip(lower=0)
                )
            ...
            # ======= SYNTHÈSE ANNUELLE (après les tables)
            with st.container(border=True):
                ...
                if "budget_formation_at" in st.session_state:
                    ...
                    df_formation["Effectif (pers.)"] = (
                        pd.to_numeric(df_formation.get("Effectif (pers.)", 0), errors="coerce")
                        .fillna(0)
                        .astype(int)
                        .clip(lower=0)
                    )
                    df_formation["Heures"] = (
                        df_formation.get("Heures", 0)
                        .apply(lambda x: float(str(x).replace(",", ".")) if pd.notna(x) else 0.0)
                        .clip(lower=0)
                    )
                    df_formation["Nbre de shifts"] = (
                        pd.to_numeric(df_formation.get("Nbre de shifts", 0), errors="coerce")
                        .fillna(0)
                        .astype(int)
                        .clip(lower=0)
                    )
                ...
                if "budget_formateurs_at" in st.session_state:
                    ...
                    df_formateurs["Effectif (pers.)"] = (
                        pd.to_numeric(df_formateurs.get("Effectif (pers.)", 0), errors="coerce")
                        .fillna(0)
                        .astype(int)
                        .clip(lower=0)
                    )
                    df_formateurs["Heures"] = (
                        df_formateurs.get("Heures", 0)
                        .apply(lambda x: float(str(x).replace(",", ".")) if pd.notna(x) else 0.0)
                        .clip(lower=0)
                    )
                    df_formateurs["Nbre de shifts"] = (
                        pd.to_numeric(df_formateurs.get("Nbre de shifts", 0), errors="coerce")
                        .fillna(0)
                        .astype(int)
                        .clip(lower=0)
                    )
                ...
                # Totaux globaux + sous-lignes
                total_heures_global = (
                    total_heures_planif + total_heures_formation + total_heures_formateurs
                )
                total_cout_global = (
                    total_cout_planif + total_cout_formation + total_cout_formateurs
                )
                ...
                with tab_cost:
                    ...
                    def _ensure_numeric_cost(df: pd.DataFrame) -> pd.DataFrame:
                        if "Coût" in df.columns:
                            df["Coût"] = (
                                pd.to_numeric(df["Coût"], errors="coerce").fillna(0.0).astype(float)
                            )
                        return df
                    ...
                    extra_rows = pd.DataFrame(
                        [
                            {
                                "Catégorie": "Formation/Doublure AT",
                                "Coût": float(total_cout_formation if "total_cout_formation" in locals() else 0.0),
                            },
                            {
                                "Catégorie": "ATF",
                                "Coût": float(total_cout_formateurs if "total_cout_formateurs" in locals() else 0.0),
                            },
                        ]
                    )
                    ...
                    if not summary.empty:
                        ...
                        with col2:
                            ...
                            rule = (
                                ...
                                .transform_aggregate(total_sel="sum(Coût_M)")
                                .mark_rule(strokeDash=[6, 4], size=2)
                                .encode(
                                    y="total_sel:Q",
                                    tooltip=[
                                        alt.Tooltip(
                                            "total_sel:Q", title="Total sélection (M CHF)", format=",.2f"
                                        )
                                    ],
                                )
                            )
                ...
                with tab_hours:
                    ...
                    if not calendar_df.empty:
                        ...
                        for c in hour_cols:
                            ...
                            total_h = (
                                pd.to_numeric(calendar_df[c], errors="coerce")
                                .fillna(0.0)
                                .sum()
                            )
                                ...
                                [
                                    ...
                                    pd.DataFrame(
                                        [
                                            {
                                                "Catégorie": "Formation/Doublure AT",
                                                "Heures": float(total_heures_formation),
                                            }
                                        ]
                                    ),
                                ],
                        ...
                        if "total_heures_formateurs" in locals():
                            heures_df = pd.concat(
                                [
                                    heures_df,
                                    pd.DataFrame(
                                        [{"Catégorie": "ATF", "Heures": float(total_heures_formateurs)}]
                                    ),
                                ],
                                ignore_index=True,
                            )
                        ...
                        with col2h:
                            ...
                            rule_h = (
                                ...
                                .transform_aggregate(total_sel="sum(Heures)")
                                .mark_rule(strokeDash=[6, 4], size=2)
                                .encode(
                                    y="total_sel:Q",
                                    tooltip=[
                                        alt.Tooltip(
                                            "total_sel:Q", title="Total sélection (h)", format=",.1f"
                                        )
                                    ],
                                )
                            )
                ...
                # --- Information additionnelle : DA OPT AT + Répartition comptes internes ---
                # S'assurer d'avoir le DF complet du calendrier
                ...
                if not calendar_df_all.empty:
                    for col in ["Heures_AT", "Heures_CSC", "Heures_EES"]:
                        if col in calendar_df_all.columns:
                            da_opt_at_heures += (
                                pd.to_numeric(calendar_df_all[col], errors="coerce")
                                .fillna(0.0)
                                .sum()
                            )

                # Ajoute la formation/doublure
                if "total_heures_formation" in locals():
                ...
                # Répartition des coûts (comptes internes) demandée
                def _sum_costs(df: pd.DataFrame, category_candidates):
                    """Somme (CHF) des colonnes Coût_<Catégorie> en testant plusieurs libellés possibles."""
                ...
                frais_surveillance = _sum_costs(
                    calendar_df_all,
                    ["Sect. FR", "Sect FR", "Secteur FR", "FR", "Frais Surveillance", "Sect_FR"],
                )

                # 318890 – Sous-traitance assistance passagers = (valeurs coût AT + EES)
                sous_traitance_assistance = _sum_costs(calendar_df_all, ["AT"]) + _sum_costs(
                    calendar_df_all, ["EES"]
                )

                # 318940 – Mandat contrôle de sûreté = (valeurs coût CSC)
                mandat_controle_surete = _sum_costs(
                    calendar_df_all, ["CSC", "Contrôle Sûreté", "Controle Securite"]
                )
            ...
            with st.container(border=True):
                ...
                if not calendar_df.empty:
                    try:
                        ...
                        monthly_summary = (
                            calendar_df[cols_to_group]
                            .groupby(["Month_Obj", "Mois_Str"], sort=True)
                        ...
                        tab_monthly_cost, tab_monthly_hour = st.tabs(
                            ["Détail Mensuel (CHF)", "Détail Mensuel (Heures)"]
                        )
                        ...
                        with tab_monthly_cost:
                            ...
                            if category_cost_cols:
                                for col in category_cost_cols:
                                    df_costs[col] = (
                                        pd.to_numeric(df_costs[col], errors="coerce")
                                        .fillna(0.0)
                                        .astype(float)
                                    )
                                df_costs["Total"] = (
                                    df_costs[category_cost_cols].sum(axis=1).astype(float)
                                )
                            else:
                                df_costs["Total"] = 0.0
                            ...
                            st.dataframe(
                                df_costs,
                                column_config=col_config_costs,
                                hide_index=True,
                                use_container_width=True,
                            )
                        ...
                        with tab_monthly_hour:
                            ...
                            if category_hour_cols:
                                for col in category_hour_cols:
                                    df_hours[col] = (
                                        pd.to_numeric(df_hours[col], errors="coerce")
                                        .fillna(0.0)
                                        .astype(float)
                                    )
                                df_hours["Total"] = (
                                    df_hours[category_hour_cols].sum(axis=1).astype(float)
                                )
                            else:
                                df_hours["Total"] = 0.0
                            ...
                            st.dataframe(
                                df_hours,
                                column_config=col_config_hours,
                                hide_index=True,
                                use_container_width=True,
                            )
                        ...
                        selected_month = st.selectbox(
                            ...
                            available_months,
                            key="budget_detail_month_select",
                        )
                        ...
                        if selected_month:
                            ...
                            with tab_daily_cost:
                                cols_to_show_cost = (
                                    ["Date", "Jour_Semaine"] + cost_cols + ["Coût_Total_Jour"]
                                )
                                ...
                                st.dataframe(
                                    df_daily_costs_view,
                                    column_config=daily_cost_config,
                                    hide_index=True,
                                    use_container_width=True,
                                )
                            ...
                            with tab_daily_hour:
                                cols_to_show_hour = (
                                    ["Date", "Jour_Semaine"] + hour_cols + ["Heures_Total_Jour"]
                                )
                                ...
                                st.dataframe(
                                    df_daily_hours_view,
                                    column_config=daily_hour_config,
                                    hide_index=True,
                                    use_container_width=True,
                                )
                ...
                with col_viz:
                    try:
                        ...
                        if not timeline_df.empty:
                            ...
                            for s in seasons:
                                colors.append(
                                    base_map.get(s, fallback_palette[fp_idx % len(fallback_palette)])
                                )
                            ...
                            timeline = (
                                ...
                                .mark_bar(cornerRadius=3, height=50)
                                .encode(
                                    ...
                                    color=alt.Color(
                                        "Saison:N",
                                        legend=alt.Legend(title="Saison"),
            ...
            if "personnel" not in st.session_state or st.session_state.personnel.empty:
                ...
                for i, key in enumerate(sorted_categories):
                    with cols[i % 3]:
                        ...
                        try:
                            default_idx = (
                                personnel_list.index(current_personnel)
                                if current_personnel in personnel_list
                                else 0
                            )
                        except ValueError:
                            default_idx = 0
                        ...
                        new_personnel = st.selectbox(
                            f"**{key}** :", personnel_list, index=default_idx, key=f"map_{key}"
                        )
