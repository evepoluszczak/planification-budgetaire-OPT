...
# ========= Mois en français (sans dépendre de la locale du système) =========
MONTHS_FR = [
    "",  # index 0 inutilisé
...
def month_name_fr(n: int, short: bool = False) -> str:
    if 1 <= n <= 12:
        return (ABBR_MONTHS_FR if short else MONTHS_FR)[n]
    return str(n)

def period_to_fr(p: pd.Period, short: bool = False) -> str:
    return f"{month_name_fr(p.month, short=short)} {p.year}"

def date_to_fr_month_label(d: dt.date, short: bool = False) -> str:
    return f"{month_name_fr(d.month, short=short)} {d.year}"
...
def _load_excel_facturation(year: int, factu_dir: Path) -> pd.DataFrame:
    ...
    for file_path in factu_dir.glob(FACTU_AT_GLOB):
        ...
        try:
            ...
            has_libelle_col, has_date_ouvrable = False, False
            for _, row in df_raw.iterrows():
                row_str = ' '.join([str(x) for x in row.values if pd.notna(x)]).lower()
                if 'libellé' in row_str and 'quantité' in row_str and 'prix' in row_str:
                    has_libelle_col = True; break
                if 'date ouvrable' in row_str:
                    has_date_ouvrable = True; break

            is_new_format = (file_year > 2025) or (file_year == 2025 and file_month >= 9)
            if has_libelle_col: is_new_format = True
            elif has_date_ouvrable: is_new_format = False
...
def _parse_new_excel_format(file_path: Path, file_year: int, file_month: int, *, show_details: bool) -> pd.DataFrame:
    ...
    if show_details:
        with st.expander(f"✓ {file_path.name} (nouveau format): {len(df)} lignes, {len(df['Type'].unique())} types"):
            ...
            details = []
            for t, g in df.groupby('Type'):
                q = g['Quantité'].sum(); mnt = g['Montant'].sum()
                details.append({
                    'Type': t,
                    'Heures': q,
                    'Coût Total (CHF)': mnt,
...
# ============================================================
# Budget Modifié (identique à besoin_jour.py)
# ============================================================
...
def calculate_budget_modifie(year: int):
    ...
    try:
        ...
        for _, r in cal.iterrows():
            ...
            h_vals.append(day_h); c_vals.append(day_h * tarif_at)
...
def _detect_invoice_format(file_path: Path) -> str:
    ...
    for _, row in df_raw.iterrows():
        ...
        if 'libellé' in s and 'quantité' in s and 'prix' in s: has_lib = True; break
        if 'date ouvrable' in s: has_date = True; break
...
# ============================================================
# Viewer d'une facture mensuelle (zone dédiée)
# ============================================================

def _view_single_invoice(year: int, month: int, factu_dir: Path):
    file_path = factu_dir / f"Facturation Lot A {month:02d}.{year}.xlsx"
    if not file_path.exists():
        st.info(f"Aucune facture trouvée pour {month_name_fr(month)} {year} ({file_path.name}).")
        return
...
def render_analyse_budgetaire_page():
    ...
    # === Visualisation d'une facture mensuelle (sans impact) ===
        ...
        with st.form(key="invoice_viewer_form", clear_on_submit=False):
            ...
            with c1:
                month_view = st.selectbox(
                    ...
                    format_func=lambda m: f"{m:02d} - {MONTHS_FR[m]}",
    ...
    # Formation / Formateurs (cohérent avec app)
    total_heures_formation = 0.0; total_cout_formation = 0.0
    cout_horaire_at = 45.50
    if 'personnel' in st.session_state and not st.session_state.personnel.empty:
        at_row = st.session_state.personnel[st.session_state.personnel['Type'] == 'AT']
        if not at_row.empty:
            try: cout_horaire_at = float(at_row['Coût Horaire'].iloc[0])
    ...
    if 'budget_formateurs_at' in st.session_state:
        ...
        df_formateurs['Total (heures)'] = df_formateurs['Effectif (pers.)'] * df_formateurs['Heures'] * df_formateurs['Nbre de shifts']
    ...
    # =================== Réalisé ===================
    heures_realise = 0.0; cout_realise = 0.0
    if not df_factu.empty:
        ...
        if t_at:
            ...
            if not pers.empty:
                r = pers[pers['Type'] == t_at]
                if not r.empty: tarif_at = float(r['Coût Horaire'].iloc[0])
        cout_realise = heures_realise * tarif_at

    # =================== KPI ===================
    ecart_heures_mod_ann = heures_modifie - heures_annuel
    ecart_heures_mod_ann_pct = (ecart_heures_mod_ann / heures_annuel * 100) if heures_annuel else 0
    ecart_cout_mod_ann = cout_modifie - cout_annuel
    ecart_cout_mod_ann_pct = (ecart_cout_mod_ann / cout_annuel * 100) if cout_annuel else 0
    ecart_heures_real_mod = heures_realise - heures_modifie
    ecart_heures_real_mod_pct = (ecart_heures_real_mod / heures_modifie * 100) if heures_modifie else 0
    ecart_cout_real_mod = cout_realise - cout_modifie
    ecart_cout_real_mod_pct = (ecart_cout_real_mod / cout_modifie * 100) if cout_modifie else 0
    ...
    with c2:
        ...
        st.markdown(f"""<div class="kpi-card kpi-amber"><div class="label">Budget Modifié</div>
    ...
    with c3:
        if df_factu.empty:
            ...
            st.markdown(f"""<div class="kpi-card kpi-red"><div class="label">Réalisé</div>
    ...
    if not calendar_with_modif.empty:
        try:
            ...
            monthly_budget['Mois_lbl'] = monthly_budget['Mois'].apply(lambda p: period_to_fr(p, short=True))
            ...
            if not df_factu.empty:
                ...
                monthly_factu['Mois_lbl'] = monthly_factu['Mois'].apply(lambda p: period_to_fr(p, short=True))
                ...
                monthly_combined = monthly_budget.merge(
                    monthly_factu[['Mois_lbl', 'Mois_dt', 'Heures_Realise', 'Cout_Realise']],
                    on=['Mois_lbl', 'Mois_dt'], how='left'
                )
            else:
            ...
            # Datasets pour graphiques (FR sur l’axe)
            df_heures_mensuel = pd.DataFrame({
                'Mois_lbl': monthly_combined['Mois_lbl'].tolist() * 3,
                'Mois_dt':  monthly_combined['Mois_dt' ].tolist() * 3,
            ...
            with tab_cout_m:
                if not df_cout_mensuel.empty:
                    ...
                    chart_cout_bars = alt.Chart(df_cout_realise).mark_bar(size=30).encode(
                        x=alt.X('Mois_lbl:N', title='Mois', sort=alt.SortField(field='Mois_dt', order='ascending')),
                        y=alt.Y('Cout:Q', title='Coût Mensuel (CHF)'),
                        color=alt.Color('Type:N', scale=alt.Scale(domain=color_domain, range=color_range),
                                        legend=alt.Legend(title='Type')),
                        tooltip=[alt.Tooltip('Mois_lbl:N', title='Mois'),
                    ...
                    st.altair_chart((chart_cout_bars + chart_cout_lines).properties(height=400).interactive(),
            ...
            with tab_heures_m:
                if not df_heures_mensuel.empty:
                    ...
                    st.altair_chart((chart_heures_bars + chart_heures_lines).properties(height=400).interactive(),
                                    use_container_width=True)
                else:
    ...
    if not calendar_with_modif.empty:
        try:
            ...
            if not df_factu.empty:
                ...
                monthly_combined = monthly_budget.merge(
                    monthly_factu[['Mois_lbl','Mois_dt','Heures_Realise_Cumul','Cout_Realise_Cumul']],
                    on=['Mois_lbl','Mois_dt'], how='left'
                )
            else:
                monthly_combined = monthly_budget.copy()
    ...
    if not calendar_with_modif.empty and 'monthly_combined' in locals():
        try:
            ...
            monthly_table['Mois'] = monthly_table['Mois_lbl']  # FR
            ...
            if not df_factu.empty:
                monthly_factu_raw = df_factu.groupby(pd.to_datetime(df_factu['Date']).dt.to_period('M')).agg({'Heures_Total':'sum'})
                monthly_factu_raw = monthly_factu_raw.reset_index().rename(columns={'index':'Mois'})
                monthly_factu_raw['Mois_lbl'] = monthly_factu_raw['Mois'].apply(lambda p: period_to_fr(p, short=True))
                ...
                right = monthly_factu_raw[['Mois_lbl', 'Heures_Total', 'Cout_Facture']].rename(
                    columns={'Mois_lbl':'__MoisKey__'}
                )

                monthly_table = monthly_table.merge(
                    right, left_on='Mois', right_on='__MoisKey__', how='left'
                )
            ...
            columns_to_show = ['Mois','Heures_Modifie','Cout_Modifie','Heures_Facturees','Cout_Facture']
            df_monthly_display = monthly_table[columns_to_show].copy()
            df_monthly_display.columns = ['Mois','Heures Modifié','Coût Modifié (CHF)','Heures Facturées','Coût Facturé (CHF)']
