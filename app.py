...
from config.constants import (
    RULES_BESOIN_JOUR_PATH,
    PAX_DATA_FILE_PATH,
    FACTU_AT_DIR,
    PAX_FORECAST_FILE_PATH,
    PAX_HISTORICAL_FILE_PATH,
)
...
from utils.async_loader import (
    start_pax_loading,
    check_pax_loading_status,
    get_pax_loading_info,
)
from utils.autosave import (
    save_session_state_auto,
    load_session_state_auto,
    autosave_exists,
    get_autosave_info,
    list_all_autosaves,
    has_session_changed,
...
# Assistant Besoin Jour (suggestions & journal)
if "assistant_suggestions" not in st.session_state:
    st.session_state.assistant_suggestions = []
if "assistant_applied_ops" not in st.session_state:
    st.session_state.assistant_applied_ops = []
if "assistant_last_apply" not in st.session_state:
    st.session_state.assistant_last_apply = None
...
if not st.session_state.data_loaded:
    ...
    with col1:
        with st.container(border=True):
            ...
            st.markdown(
                "Charge les données de référence de 2026 pour commencer une nouvelle planification."
            )
            if st.button(
                "Démarrer avec la base 2026", use_container_width=True, type="primary"
            ):
                try:
                    ...
                    st.error(
                        f"Erreur lors de l'initialisation des données 2026: {e}"
                    )
    ...
    with col2:
        with st.container(border=True):
            ...
            st.markdown(
                "Chargez un fichier `.xlsx` que vous avez précédemment sauvegardé."
            )
        ...
        with col3:
            with st.container(border=True):
                ...
                if all_autosaves:
                    ...
                    if st.button(
                        "Reprendre la session", use_container_width=True, type="secondary"
                    ):
    ...
    with st.sidebar:
        ...
        if st.button(
            ...
            use_container_width=True,
            type="primary"
            if st.session_state.selected_page == "Configuration"
            else "secondary",
        ):
        ...
        if st.button(
            ...
            use_container_width=True,
            type="primary"
            if st.session_state.selected_page == "Planification"
            else "secondary",
        ):
        ...
        if st.button(
            ...
            use_container_width=True,
            type="primary"
            if st.session_state.selected_page == "Budget Annuel"
            else "secondary",
        ):
        ...
        if st.button(
            ...
            use_container_width=True,
            type="primary"
            if st.session_state.selected_page == "Besoin Jour"
            else "secondary",
        ):
        ...
        if st.button(
            ...
            use_container_width=True,
            type="primary"
            if st.session_state.selected_page == "Analyse Budgétaire"
            else "secondary",
        ):
        ...
        if st.button(
            ...
            use_container_width=True,
            type="primary"
            if st.session_state.selected_page == "Comparaison Historique"
            else "secondary",
        ):
        ...
        if st.button(
            ...
            use_container_width=True,
            type="primary"
            if st.session_state.selected_page == "Simulateur Objectif"
            else "secondary",
        ):
            change_page("Simulateur Objectif")

        # Assistant Besoin Jour (nouvelle page)
        if st.button(
            "Assistant Besoin Jour",
            use_container_width=True,
            type="primary"
            if st.session_state.selected_page == "Assistant Besoin Jour"
            else "secondary",
        ):
            change_page("Assistant Besoin Jour")

        # Compteur de suggestions en attente (optionnel)
        pending = len(st.session_state.get("assistant_suggestions", []))
        if pending:
            st.caption(f"🧠 Suggestions en attente : {pending}")
        ...
        if "last_autosave" in st.session_state:
            try:
                ...
                if last_save.tzinfo is None:
                    last_save = last_save.replace(tzinfo=ZoneInfo("UTC")).astimezone(
                        paris_tz
                    )
                else:
        ...
        # Dernière application de l’assistant (optionnel)
        if st.session_state.get("assistant_last_apply"):
            try:
                when = st.session_state.assistant_last_apply.get("when", "—")
                st.caption(f"📝 Dernière application de l’assistant : {when}")
            except Exception:
                pass
        ...
        def pax_loading_status_fragment():
            ...
            if loading_status == "loading":
                ...
                if not st.session_state.pax_error_dismissed:
                    ...
                    st.caption(
                        "Erreur de chargement masquée (cliquez Recharger pour réessayer)."
                    )
        ...
        if loading_status == "idle":
            ...
            if st.button(
                "🔄 Lancer le chargement PAX", use_container_width=True, type="secondary"
            ):
                ...
                if not forecast_exists and not historical_exists:
                    ...
                    if not forecast_exists:
                        st.warning(
                            f"Forecast_pax.xlsx non trouvé, chargement Historic uniquement"
                        )
                    if not historical_exists:
                        st.warning(
                            f"Historic_pax.xlsx non trouvé, chargement Forecast uniquement"
                        )
        ...
        with st.form("export_form"):
            ...
            submitted = st.form_submit_button(
                "Préparer le Fichier", use_container_width=True, type="primary"
            )
            ...
            if submitted:
                ...
                try:
                    ...
                    st.success(
                        f"Fichier '{filename}' prêt. Cliquez sur 'Télécharger'."
                    )
                except Exception as e:
    ...
    if page == "Configuration":
        ...
        # Nouvelle page Assistant (nécessite le module fourni)
        from ui.pages.besoin_jour_assistant import (
